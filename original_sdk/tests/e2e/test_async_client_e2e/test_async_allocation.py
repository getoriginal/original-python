...
from original_sdk import OriginalAsyncClient, ClientError
...
class TestClientE2E:
    ...
    async def test_get_allocation(self, async_client: OriginalAsyncClient):
        response = await async_client.get_allocation(gbl.env_data["test_allocation_uid"])
        assert response["data"]["uid"] == gbl.env_data["test_allocation_uid"]

    async def test_get_allocation_not_found_throws_404(self, async_client: OriginalAsyncClient):
        try:
    ...
    async def test_get_allocations_by_user_uid(self, async_client: OriginalAsyncClient):
        response = await async_client.get_allocations_by_user_uid(gbl.env_data["test_app_user_uid"])
        assert isinstance(response["data"], list)

    async def test_get_allocations_by_user_uid_with_no_results(self, async_client: OriginalAsyncClient):
