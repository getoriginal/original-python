...
class TestClientE2E:
    ...
    async def test_get_claim_not_found_throws_404(self, async_client: OriginalAsyncClient):
        try:
    ...
    async def test_get_claims_by_user_uid(self, async_client: OriginalAsyncClient):
        response = await async_client.get_claims_by_user_uid(gbl.env_data["test_app_user_uid"])
        assert isinstance(response["data"], list)

    async def test_get_claims_by_user_uid_with_no_results(self, async_client: OriginalAsyncClient):
